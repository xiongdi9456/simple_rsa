          ...
          children: <Widget>[
              ...
              children: <Widget>[
                RaisedButton(
                  ...
                  onPressed: () async {
                    String tmp = "";
                    try {
                      if (_stringSigned.isEmpty) {
                        final text = myController.text;
                        if (text.isNotEmpty) {
                          print("sign with privatekey:"+myController.text);
                          final tmp = await signString(myController.text, PRIVATE_KEY);
                          setState(() {
                            _stringSigned = tmp;
                          });
                        }else{
                          setState(() {
                            myController.text = "Need to input string";
                          });
                        }
                      }
                    } catch (e) {
                ...
                RaisedButton(
                    ...
                    onPressed: () async {
                      bool tmp = false;
                      if (_stringSigned.isNotEmpty) {
                        try {
                          tmp = await verifyString(myController.text , _stringSigned, PUBLIC_KEY);
                          print("tmp:{$tmp}");
                          setState(() {
                            _stringSigned = "${tmp}";
                          });
            ...
            Row(
              ...
              children: <Widget>[
                  ...
                  onPressed: () async {
                    ...
                    try {
                      if (_stringSigned.isEmpty) {
                        final text = myController.text;
                        if (text.isNotEmpty) {
                          print("signRSA2 with privatekey:"+myController.text);
                          final tmp = await signStringRSA2(myController.text, PRIVATE_KEY);
                          setState(() {
                            _stringSigned = tmp;
                          });
                        }else{
                ...
                RaisedButton(
                    child: Text("VerifyRSA2"),
                    color: Colors.red,
                    onPressed: () async {
                      bool tmp = false;
                      if (_stringSigned.isNotEmpty) {
                        try {
                          tmp = await verifyStringRSA2(myController.text , _stringSigned, PUBLIC_KEY);
                          print("tmp:{$tmp}");
                          setState(() {
                            _stringSigned = "${tmp}";
                          });
            ...
            Padding(
              ...
              child: Card(
                ...
                child: Padding(
                  ...
                  child: Column(
                    children: <Widget>[
                      Text("STRING Sign And Verify Result"),
                      Divider(),
                      Text(_stringSigned),
                    ],
                  ),
